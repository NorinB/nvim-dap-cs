...
local project_selection = function(project_path, allow_multiple)
	local check_csproj_cmd = string.format('find %s -type f -name "*.csproj" && echo', project_path)
...
local select_dll = function(project_path)
	local bin_path = project_path

	local check_net_folders_cmd = string.format('find %s -type d -name "net*" -path "*/bin/*"', bin_path)
